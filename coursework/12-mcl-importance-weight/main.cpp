...
int main()
{
    ...
    //DONE: Generate particle weights depending on robot's measurement
    //DONE: Print particle weights, each on a single line
    double w[n];
 
    // NOTE: Particles are calculating their weight by comparing to
    // their measurement to that of the robot (z)
    for (int i = 0; i < n; i++) {
